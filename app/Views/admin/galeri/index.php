              ...
              <div class="card card-success">
                ...
                <div class="card-body">
                  <table id="datatable" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      <?php if ($galeri != null) {
                        foreach ($galeri as $key => $v) { ?>
                          <tr>
                            <td><?= ++$key ?></td>
                            <td><?= $v->nama ?></td>
                            <td><img src="<?= base_url('uploads/galeri/' . $v->file) ?>" alt="<?= $v->nama ?>" style="height:64px"></td>
                            <td>
                              <a onclick="return confirm('Hapus Data?\nTindakan ini tidak dapat diurungkan.')" href="<?= site_url('admin/galeri/hapus/' . $v->id_galeri) ?>" class="btn btn-sm btn-danger" title="Hapus"><i class="fa fa-trash"></i></a>
                            </td>
                          </tr>
                      <?php }
                      } ?>
